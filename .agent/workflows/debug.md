...
// Get board settings
TrelloPowerUp.iframe().get('board', 'shared', 'boardSettings').then(console.log);
...
await t.set('card', 'shared', 'timerData', {
    entries: [
        { id: 'test1', startTime: Date.now() - 3600000, endTime: Date.now(), duration: 3600000, description: '', createdAt: Date.now() }
    ],
    state: 'idle',
    currentEntry: null,

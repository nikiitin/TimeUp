...
jest.unstable_mockModule('../../src/services/ChecklistService.js', () => ({ default: mockChecklistService }));

const { EstimateUI } = await import('../../src/ui/EstimateUI.js');
const { default: TimerService } = await import('../../src/services/TimerService.js');
const { default: ChecklistService } = await import('../../src/services/ChecklistService.js');
...
describe('EstimateUI', () => {
    ...
    test('handles invalid input', async () => {
        ...
        expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('Invalid time format'));
    ...
    test('update shows progress bar when estimate exists', () => {
        const timerData = { manualEstimateSet: true, estimatedTime: 3600000, entries: [] };
